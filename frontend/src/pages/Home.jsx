...
const Home = () => {
  ...
  const handleCreateMeeting = async () => {
    ...
    const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/meetings/create`, {
      method: "POST",
  ...
  const handleJoinMeeting = async (e) => {
    e.preventDefault();
    if (meetingId.trim()) {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/meetings/join/${meetingId}`);
      const data = await res.json();
      if (data.success) {
        navigate(`/${meetingId}`);
      } else {
